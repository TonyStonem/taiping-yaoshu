...
//                .cache(cache)
                ...
                try {
                    ...
                    if (response == null) {
                        handler.post(new Runnable() {
                            @Override
                            public void run() {
                                Logger.e("Request failed::code=" + -1 + ",msg=" + "response null"
                                        + ",url=" + reqUrl);
                    ...
                    if (response.isSuccessful()) {
                        ...
                        handler.post(new Runnable() {
                            @Override
                            public void run() {
                                Logger.e("Request failed::code=" + code + ",msg=" + msg + ",url="
                                        + reqUrl);
                    ...
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            Logger.e("Request failed::code=" + -1 + ",msg=" + e.getMessage() + "," +
                                    "url=" + reqUrl);
    ...
    private static void addHeadersToRequest(ArrayMap<String, String> headers, Request.Builder
            builder) {
    ...
    private static <T> void basePost(String url, ArrayMap<String, String> headers, JsonObject
            params, Object tag, OnResponse<T> onResponse) {
        final RequestBody reqBody = RequestBody.create(JSON, params == null ? EMPTY_JSON : params
                .toString());
    ...
    public static <T> void postForm(String url, ArrayMap<String, String> params, Object tag,
...
//        reqBuilder.addHeader("Authorization", "token"/*这里可以添加授权的头信息*/);
    ...
    public static <T> void postNoToken(String url, ArrayMap<String, String> headers, JsonObject
            params, Object tag, OnResponse<T> onResponse) {
        basePost(url, headers, params, tag, onResponse);
    }

    private static <T> void post(String url, ArrayMap<String, String> headers, JsonObject params,
                                 Object tag, OnResponse<T> onResponse) {
        if (headers == null) {
...
//        headers.put("Authorization", ConstantValues.TOKEN_STRING + GlobalParams.token);
    ...
    public static <T> void post(String url, JsonObject params, Object tag, OnResponse<T>
            onResponse) {
    ...
    public static <T> void getNoToken(String url, ArrayMap<String, String> headers,
...
//        builder.header("Authorization", ConstantValues.TOKEN_STRING + GlobalParams.token);
    ...
    public abstract static class OnResponse<T> {
        ...
        public void onFailed(int code, String msg, String url) {
            switch (code) {
                case -1:
                    UIUtils.showToast("网络错误，请稍候重试");
                    break;
                case HttpURLConnection.HTTP_CLIENT_TIMEOUT:
                    UIUtils.showToast("连接超时，请稍候重试");
                    break;

                case HttpURLConnection.HTTP_INTERNAL_ERROR:
                    UIUtils.showToast("服务器忙，请稍候重试");
                    break;
